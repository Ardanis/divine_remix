...
ADD_TRANS_ACTION BHARVAL BEGIN 22 END BEGIN 3 END ~EraseJournalEntry(@3) EraseJournalEntry(@4)~
...
IF WEIGHT #-100 ~Global("CDSpawnArval","GLOBAL",2)
                 ...
                 InPartySlot(LastTalkedToBy,0)
                 Global("BeholderPlot","GLOBAL",0)
                 Global("NoHelpBeholder","GLOBAL",0)~ THEN BEGIN CDOisigRefer SAY @5
  IF ~ReputationGT(Player1,14)
      Global("CDFakeKossF","GLOBAL",0)~ THEN REPLY @6 GOTO CDGood
  IF ~OR(2)
        ReputationLT(Player1,15)
        Global("CDFakeKossT","GLOBAL",1)~ THEN REPLY @6 GOTO CDMeh
  IF ~ReputationGT(Player1,14)
      Global("CDFakeKossF","GLOBAL",0)~ THEN REPLY @7 GOTO CDGoodWait
  IF ~OR(2)
        ReputationLT(Player1,15)
        Global("CDFakeKossT","GLOBAL",1)~ THEN REPLY @7 GOTO CDMehWait
  IF ~ReputationGT(Player1,14)
      Global("CDFakeKossF","GLOBAL",0)~ THEN REPLY @8 GOTO CDGood
  IF ~OR(2)
        ReputationLT(Player1,15)
        Global("CDFakeKossT","GLOBAL",1)~ THEN REPLY @8 GOTO CDMeh
END

IF ~~ THEN BEGIN CDGood SAY @9
  IF ~~ THEN GOTO 4
END

IF ~~ THEN BEGIN CDMeh SAY @10
  IF ~~ THEN GOTO 4
END

IF ~~ THEN BEGIN CDGoodWait SAY @11
  IF ~~ THEN GOTO 4
END

IF ~~ THEN BEGIN CDMehWait SAY @12
  IF ~~ THEN GOTO 4
END

IF ~~ PerhapsHelpHelm SAY @1
  IF ~~ THEN DO ~OpenDoor("DOR0902b")
                 EscapeAreaMove("AR0902",577,650,14)
                 SetGlobal("CDSpawnOisig","GLOBAL",1)~ UNSOLVED_JOURNAL @3 EXIT
END
END
...
ADD_TRANS_ACTION BHNALLA BEGIN 21 END BEGIN 3 END ~EraseJournalEntry(@3) EraseJournalEntry(@4)~
...
ADD_TRANS_ACTION BHOISIG BEGIN 24 END BEGIN 3 END ~EraseJournalEntry(@3) EraseJournalEntry(@4)~
...
IF WEIGHT #-100 ~Global("CDSpawnOisig","GLOBAL",2)
                 ...
                 Global("BeholderPlot","GLOBAL",0)
                 Global("NoHelpBeholder","GLOBAL",0)
                 !Kit(Player1,GODLATHANDER)~ THEN BEGIN CDArvalRefer SAY @13
  IF ~ReputationGT(Player1,14)~ THEN REPLY @14 GOTO CDGood
  IF ~ReputationLT(Player1,15)~ THEN REPLY @14 GOTO CDMeh
  IF ~ReputationGT(Player1,14)~ THEN REPLY @7 GOTO CDGoodWait
  IF ~ReputationLT(Player1,15)~ THEN REPLY @7 GOTO CDMehWait
  IF ~ReputationGT(Player1,14)~ THEN REPLY @15 GOTO CDGood
  IF ~ReputationLT(Player1,15)~ THEN REPLY @15 GOTO CDMeh
END

IF ~~ THEN BEGIN CDGood SAY @16
  IF ~~ THEN GOTO 8
END

IF ~~ THEN BEGIN CDMeh SAY @17
  IF ~~ THEN GOTO 8
END

IF ~~ THEN BEGIN CDGoodWait SAY @18
  IF ~~ THEN GOTO 8
END

IF ~~ THEN BEGIN CDMehWait SAY @19
  IF ~~ THEN GOTO 8
END

IF ~~ PerhapsHelpLathander SAY @2
  IF ~~ THEN DO ~OpenDoor("DOOR0901")
                 EscapeAreaMove("AR0901",690,565,2)
                 SetGlobal("CDSpawnArval","GLOBAL",1)~ UNSOLVED_JOURNAL @4 EXIT
END
END
